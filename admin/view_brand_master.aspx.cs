...
public partial class Admin_view_brand_master : System.Web.UI.Page
{
    ...
    private void SearchGrid(string searchField, string pname)
    {
        ...
        obj.SetCommandQry = "select * from brand_master_tbl  where " + searchField + " like  '%'+'" + pname + "'+ '%' ";
    ...
    private void SearchGrid(string searchField)
    {
        DataTable dt = new DataTable();
        DBconnection obj = new DBconnection();
        obj.SetCommandQry = "select * from brand_master_tbl where brand_name='" + searchField + "'";
        dt = obj.ExecuteTable();
        if (dt.Rows.Count > 0)
        {
            lblMsg.Visible = false;
