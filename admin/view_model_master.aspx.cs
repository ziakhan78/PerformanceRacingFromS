...
public partial class Admin_view_model_master : System.Web.UI.Page
{
    ...
    private void SearchGrid(string searchField, string pname)
    {
        ...
        obj.SetCommandQry = "select * from View_ManufacturerModel  where " + searchField + " like  '%'+'" + pname + "'+ '%' ";
    ...
    private void SearchByAlphabet(string name)
    {
        ...
        dt = obj.SearchManufacturerByAlphabetModel();
    ...
    private void SearchGrid(string searchField)
    {
        DataTable dt = new DataTable();
        DBconnection obj = new DBconnection();
        obj.SetCommandQry = "select * from View_ManufacturerModel where manufacturer_name='" + searchField + "'";
        dt = obj.ExecuteTable();
        if (dt.Rows.Count > 0)
        {
            lblMsg.Visible = false;
    ...
    private void BindGrid()
    {
        DataTable dt = new DataTable();
        MastersBLL obj = new MastersBLL();
        dt = obj.GetManufacturerModelList();
        if (dt.Rows.Count > 0)
        {
            lblMsg.Visible = false;
            RadGrid1.Visible = true;
    ...
    private void ManageGrid()
    {
        try
        {
            ...
            {
                SearchGrid(Session["SearchValue"].ToString());
                RadGrid1.CurrentPageIndex = Convert.ToInt16(Request.Cookies["currentpage"]["pageIndex"].ToString());
                Request.Cookies["currentpage"].Expires = DateTime.Now.AddDays(-1);
            }
    ...
    protected void RadGrid1_ItemCommand(object sender, Telerik.Web.UI.GridCommandEventArgs e)
    {
        ...
        if (e.CommandName == "Delete")
        {
            ...
            int exe = obj.DeleteManufacturerModel();
    ...
    {
        SearchGrid(DDLManufacturer.SelectedItem.Text.Trim());
    }
}