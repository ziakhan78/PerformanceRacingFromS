...
public partial class Admin_view_components : System.Web.UI.Page
{
    ...
    private void SearchGrid(string searchField, string pname)
    {
        ...
        obj.SetCommandQry = "select * from components_master_tbl  where " + searchField + " like  '%'+'" + pname + "'+ '%'  ";
    ...
    private void SearchByAlphabet(string name)
    {
        ...
        obj.Component = name;
        dt = obj.SearchComponentsByAlphabet();
    ...
    private void SearchGrid(string searchField)
    {
        DataTable dt = new DataTable();
        DBconnection obj = new DBconnection();
        obj.SetCommandQry = "select * from components_master_tbl where components='" + searchField + "'";
        dt = obj.ExecuteTable();
        if (dt.Rows.Count > 0)
        {
            lblMsg.Visible = false;
